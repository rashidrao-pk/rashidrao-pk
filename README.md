# Muhammad Rashid

Machine Learning Researcher | Computer Vision | Explainable AI | Anomaly Detection

![GitHub followers](https://img.shields.io/github/followers/rashidrao-pk?style=social)
![GitHub stars](https://img.shields.io/github/stars/rashidrao-pk?style=social)
![AAAI](https://img.shields.io/badge/AAAI-2026-orange)
![AAAI](https://img.shields.io/badge/AAAI-2024-orange)
![XAI](https://img.shields.io/badge/XAI-Research-blue)
![Focus](https://img.shields.io/badge/Focus-Explainable%20AI-green)
[![Website](https://img.shields.io/badge/Website-Visit-blue)](https://rashidrao-pk.github.io/) ![Field](https://img.shields.io/badge/Field-Computer%20Vision-purple) ![PhD](https://img.shields.io/badge/PhD-Computer%20Science-red)

I am a PhD researcher at the **University of Turin, Italy**, working on **Computer Vision**, **eXplainable AI (XAI)**, and **visual anomaly detection** for industrial applications.

My recent work focuses on:
- explainable computer vision
- anomaly detection
- feature attribution methods
- trustworthy AI
- image-based machine learning systems



## Research Highlights

- **ShapBPT (AAAI 2026)**  
  Image feature attributions using data-aware binary partition trees  
  - [Paper](https://ojs.aaai.org/index.php/AAAI/article/view/39699), [arXiv](https://arxiv.org/abs/2602.07047), [Code](https://github.com/amparore/shap_bpt), [Tests](https://github.com/rashidrao-pk/shap_bpt_tests), [PyPI](https://pypi.org/project/shap-bpt/)

- **Using Stratified Sampling to Improve LIME Image Explanations (AAAI 2024)**  
  - [Paper](https://ojs.aaai.org/index.php/AAAI/article/view/29397), [Code](https://github.com/rashidrao-pk/lime_stratified), [Examples](https://github.com/rashidrao-pk/lime-stratified-examples), [PyPI](https://pypi.org/project/lime-stratified/)

- **Can I Trust My Anomaly Detection System? A Case Study Based on Explainable AI (XAI-World 2024)**  
  - [Paper](https://link.springer.com/chapter/10.1007/978-3-031-63803-9_13), [Code](https://github.com/rashidrao-pk/anomaly_detection_trust_case_study)


## Featured Repositories

- [**_shap_bpt_tests_**](https://github.com/rashidrao-pk/shap_bpt_tests) — experiments and evaluation for ShapBPT
- [**_lime_stratified_**](https://github.com/rashidrao-pk/lime_stratified) — improved LIME image explanations with stratified sampling
- [**_anomaly_detection_trust_case_study_**](https://github.com/rashidrao-pk/anomaly_detection_trust_case_study) — explainable anomaly detection case study

## Selected Publications

1. **ShapBPT: Image Feature Attributions using Data-Aware Binary Partition Trees**  
   AAAI 2026

2. **Using Stratified Sampling to Improve LIME Image Explanations**  
   AAAI 2024

3. **Can I Trust My Anomaly Detection System? A Case Study Based on Explainable AI**  
   XAI-World 2024

4. **A Sustainable Deep Learning Framework for Object Recognition Using Multi-Layers Deep Features Fusion and Selection**  
   Sustainability, 2020

5. **Object Detection and Classification: A Joint Selection and Fusion Strategy of Deep Convolutional Neural Network and SIFT Point Features**  
   Multimedia Tools and Applications, 2018

## Links

- [Website/GtHub Page](https://rashidrao-pk.github.io/)
- [Google Scholar](https://scholar.google.com.pk/citations?user=F5u_Z5MAAAAJ)
- [ORCID](https://orcid.org/0000-0002-2557-6845)
- [ResearchGate](https://www.researchgate.net/profile/Muhammad-Rashid-65)
- [Scopus](https://www.scopus.com/authid/detail.uri?authorId=57221707362&origin=resultslist)
- [LinkedIn](https://www.linkedin.com/in/r4ssh1d)
- [PyPI](https://pypi.org/user/mrashid/)
- [Kaggle](https://www.kaggle.com/rashidrao)

## About Me

My broader background includes research and applied projects in:
- computer vision
- object detection
- medical imaging
- web development
- machine learning for industrial systems

For a full profile, publications list, talks, and project archive, visit my website:

**[rashidrao-pk.github.io](https://rashidrao-pk.github.io/)**

## 📊 GitHub Stats


<img src="https://github-readme-activity-graph.vercel.app/graph?username=rashidrao-pk&hide_border=true"/>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=rashidrao-pk&show_icons=true&hide_title=true&hide_border=true&count_private=true" height="150"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rashidrao-pk&layout=compact&hide_border=true" height="150"/>
</p>

---

<div style="text-align: right;">
  <a href="#" 
     style="
        display: inline-block; 
        padding: 5px 10px; 
        background-color: #469B60; 
        color: white; 
        text-decoration: none; 
        border-radius: 5px; 
        font-weight: bold; 
        font-size: 14px; 
        box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.2);">
    Go to TOP
  </a>
</div>